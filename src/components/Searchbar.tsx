...
export default function Searchbar({
  ...
  const focusedSearchBarId = useSearchStore(
    (state) => state.focusedSearchBarId,
  );
  ...
  const handleSearch = () => {
    ...
    if (trimmed) {
      router.push(`/today-news/search?keyword=${encodeURIComponent(trimmed)}`);
    }
     setQuery(focusedSearchBarId, "");
  };
    ...
    <div className={`relative ${className}`}>
      <div className="input-gradient-border relative h-full w-full from-purple-500 to-green-500">
        <input
          ...
          value={focusedSearchBarId === id ? query : ""}
